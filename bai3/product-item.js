class productOne extends HTMLElement {
    ...
    static get observedAttributes() {
      return ['image', 'time', 'title', 'content_1', 'content_row3', 'logo']; 
    }
    // cách 1 gọi hàm đặt hàm là const abc = this.stribute.abc;
    ...
    renderHTML() {
      ...
      <div class="layout">
        <div class="row1">
        <img src="${this.attr('logo')}" alt="logo" class="logo" >
        <p class="content-row1">${this.attr('content_row1')}</p>
        </div>
        <div class="row2">
        <img src="${this.attr('image')}" alt="thulnall" class="image">
        </div>
        <div class="row3">
        <p>${this.attr('time')}</p>
        <p>${this.attr('content_3')}</p>
        <p> Alertsnew.net</p>
        <div class="row4">
